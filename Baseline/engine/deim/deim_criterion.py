...
class DEIMCriterion(nn.Module):
    ...
    def __init__(self, \
        ...
        if self.no_weight_vfl_epoch != -1:
            print( f"no_weight_vfl_epoch set {self.no_weight_vfl_epoch}")

    def loss_distillation(self, outputs, targets, indices, num_boxes, **kwargs):
        # if self.epoch == 0:
        #     print("[CHECK-1] loss_distillation called")
        ...
        if student_feature_map is None or teacher_feature_map is None:
            # print("❌ [DISTILL] missing student or teacher")
            return {}
        ...
        # print("student is None:", outputs.get('student_distill_output') is None)
        # print("teacher is None:", outputs.get('teacher_encoder_output') is None)

        # _logger.info(f"[Criterion] Student feature map shape: {student_feature_map.shape}")
        # _logger.info(f"[Criterion] Teacher feature map shape: {teacher_feature_map.shape}")

        if student_feature_map.shape[1] != teacher_feature_map.shape[1]:
            _logger.error(
                f"[Criterion] Feature dimension mismatch! Student: {student_feature_map.shape[1]}, Teacher: {teacher_feature_map.shape[1]}")
        ...
        if (H_s, W_s) != (H_t, W_t):
            _logger.warning(
                f"[Criterion] Resizing teacher feature map from {H_t}x{W_t} to student's {H_s}x{W_s} for distillation.")
        ...
        # if self.epoch == 0:
        #     print("[CHECK-2] teacher requires_grad =", teacher_feature_map.requires_grad)
        ...
        ###################################ROI/前景加权################################
        cos_sim = F.cosine_similarity(student_output_norm, teacher_output_norm, dim=-1)  # [B, HW]
        distill_map = (1.0 - cos_sim)  # [B, HW]

        B, HW = distill_map.shape
        device = distill_map.device
        H, W = target_h, target_w  # student feature map spatial size

        # ---- hyper-params (start with these) ----
        bg_weight = 0.1  # background weight; try 0.05~0.1
        dilate = 1  # expand ROI by N tokens; try 0/1/2

        weights = torch.full((B, HW), bg_weight, device=device, dtype=distill_map.dtype)

        for b in range(B):
            if ('boxes' not in targets[b]) or (targets[b]['boxes'].numel() == 0):
                continue

            # targets[b]['boxes'] is cxcywh normalized [0,1]
            ...
            # clamp + dilate on token grid
            x1 = torch.floor(x1 - dilate).clamp(0, W - 1).long()
            y1 = torch.floor(y1 - dilate).clamp(0, H - 1).long()
            x2 = torch.ceil(x2 + dilate).clamp(0, W - 1).long()
            y2 = torch.ceil(y2 + dilate).clamp(0, H - 1).long()

            fg = torch.zeros((H, W), device=device, dtype=torch.bool)
            for i in range(boxes.shape[0]):
                fg[y1[i]:y2[i] + 1, x1[i]:x2[i] + 1] = True

            fg_flat = fg.view(-1)
            if fg_flat.any():
                weights[b, fg_flat] = 1.0  # foreground weight

        loss_distill = (distill_map * weights).sum() / (weights.sum() + 1e-6)
        ###########################################################################

        return {'loss_distill': loss_distill}

    def _get_distillation_weight_for_epoch(self) -> float:
        base = self.weight_dict.get('loss_distill', 0.0)
        # warmup_epochs = 2  # 建议 1 或 2
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        for loss in self.losses:
            if loss == 'distill':
                l_dict = self.get_loss(loss, outputs, targets, None, None, **kwargs)
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    if loss in ['ccm', 'distill']:  # ✅ 跳过 distill
                        continue
        ...
        # if epoch == 0:
        #     print("[CHECK-3] losses keys:", losses.keys())
        #     if 'loss_distill' in losses:
        #         print("[CHECK-3] loss_distill =", float(losses['loss_distill']))
    ...
    def set_epoch(self, epoch):   
        ...
        if self.no_weight_vfl_epoch != -1 and self.epoch == self.no_weight_vfl_epoch: 
            print(f"Epoch:[{self.epoch}]>=[{self.no_weight_vfl_epoch}] using weight in vfl/mal." )