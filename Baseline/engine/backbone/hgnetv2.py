...
import os
from engine.backbone.common import FrozenBatchNorm2d
...
class LightConvBNAct(nn.Module):
    def __init__(
        ...
        self.conv2 = ConvBNAct(
            out_chs, out_chs, kernel_size=kernel_size, groups=out_chs,
...
class EseModule(nn.Module):
    def __init__(self, chs):
        super().__init__()
        self.conv = nn.Conv2d(chs, chs, kernel_size=1, stride=1)
    ...
    def __init__(self,
        ...
        if pretrained:
            try:
                ...
                if os.path.exists(model_path):
                    ...
                    if torch.distributed.is_available() and torch.distributed.is_initialized():
                        if torch.distributed.get_rank() == 0:
                            print("Downloading pretrained HGNetV2...")
                ...
                print(f"{str(e)}")
                logging.error("CRITICAL WARNING: Failed to load pretrained HGNetV2 model")
                logging.error(f"Please manually download from {download_url} to {local_model_dir}")
